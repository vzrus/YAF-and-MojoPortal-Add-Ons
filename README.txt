...
2. Put the YAF folder into MP 'Web' folder. 

3. Merge configs - lokk for example working config in the folder. Just replace you connection strings. For further details go to MP site. Fix merged web.config
Set this value in web.config  
 <add key="YAF.ConfigPassword" value="yourpass" /> It will be required to enter while you install YAF.
Change connection string (1 for YAF and the second for MP)
Don't use user.config in the folder - it's for reference only.

4.Remove old YAF Web.config, db.config and app.config from the folder. Move all assemblies from bin folder to MP bin folder and remove YAF bin folder to not publish unused files.

5. Fix assemblies references - they should poin to assemblies in MP bin folder. Compile it.


6. Before install!
YAF stores passwords encripted - make it for MP admin too before install just in case.

Now you can simply add the module to MP, when you first time go to module setting it'll redirect you to YAF install page. You should install YAF. 
...
7.Click on Host on the upper panel. Check Enable Display Name: on Features tab.
...
It's recommended to create a second board in the module as it will be set for you automatically.

...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
			...
			DELETE FROM [{databaseOwner}].[{objectQualifier}prov_Profile] WHERE UserName = @UserName AND ApplicationID = @ApplicationID
...
BEGIN
    ...
    INSERT INTO #PageIndexForUsers (UserID)
        ...
        WHERE   ApplicationID = @ApplicationID           
            ...
            AND (@UserNameToMatch IS NULL OR UserName LIKE @UserNameToMatch)
        ORDER BY UserName


    SELECT  p.*
    FROM    [{databaseOwner}].[{objectQualifier}prov_Profile] p, #PageIndexForUsers i
    WHERE   p.UserId = i.UserId AND i.IndexId >= @PageLowerBound AND i.IndexId <= @PageUpperBound
...
BEGIN
    ...
    SELECT  COUNT(*)
    FROM   [{databaseOwner}].[{objectQualifier}prov_Profile] p
    WHERE   p.ApplicationID = @ApplicationID      
        AND (p.LastActivity <= @InactiveSinceDate)
END
GO
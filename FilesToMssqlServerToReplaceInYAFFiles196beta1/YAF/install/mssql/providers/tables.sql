...
if exists(select top 1 1 from syscolumns where id=object_id('[{databaseOwner}].[{objectQualifier}prov_Profile]') and name='UserID' and xtype='36')
begin
	if exists(select top 1 1 from dbo.sysindexes where id=object_id('[{databaseOwner}].[{objectQualifier}prov_Profile]') and (status & 2048) <> 0)
	begin
		-- drop the primary key constrant
		DECLARE @PrimaryIXName nvarchar(255)		
...
if not exists(select top 1 1 from syscolumns where id=object_id('[{databaseOwner}].[{objectQualifier}prov_Profile]') and name='LastActivity')
begin
	ALTER TABLE [{databaseOwner}].[{objectQualifier}prov_Profile] ADD [LastActivity] [datetime]	
end
GO
...
if exists(select top 1 1 from syscolumns where id=object_id('[{databaseOwner}].[{objectQualifier}prov_RoleMembership]') and name='UserID' and xtype='36')
begin
	-- drop the provider user key index if it exists...
	if exists(select top 1 1 from dbo.sysindexes where name=N'IX_{objectQualifier}prov_RoleMembership_UserID' and id=object_id(N'[{databaseOwner}].[{objectQualifier}prov_RoleMembership]'))
	begin
